...
const Nav = ({ onRequestAssessment }: { onRequestAssessment: () => void }) => (
  <nav className="fixed top-0 w-full z-50 px-6 py-4 flex justify-between items-center border-b border-border bg-background/80 backdrop-blur-sm">
    ...
    <Button
      ...
      size="sm"
      className="bg-highlight text-highlight-foreground hover:bg-highlight/90 rounded-sm text-xs tracking-wide"
    >
      Request Assessment
    </Button>
  </nav>
...
const Hero = ({ onRequestAssessment }: { onRequestAssessment: () => void }) => (
  <section className="min-h-screen flex items-center justify-center px-6 relative overflow-hidden pt-16">
    <div className="absolute right-0 top-1/2 -translate-y-1/2 w-[45%] max-w-lg opacity-30 pointer-events-none select-none">
      <img
        src={romanBust}
        alt=""
    ...
    <div className="relative z-10 text-center flex flex-col items-center">
      ...
      <FadeIn delay={600}>
        <div className="flex flex-col sm:flex-row gap-4 mb-16">
          <Button
            onClick={onRequestAssessment}
            className="bg-highlight text-highlight-foreground hover:bg-highlight/90 rounded-sm px-8 py-3 text-sm tracking-wide"
          >
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {[
        ...
        <FadeIn key={col.num} delay={200 * i}>
          <div className="border border-border rounded-sm bg-surface p-6 hover:border-accent/40 transition-colors h-full">
            <Collapsible>
...
const Engagement = ({ onRequestAssessment }: { onRequestAssessment: () => void }) => (
  <section className="px-6 py-32 max-w-5xl mx-auto">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <FadeIn delay={200}>
        <div className="border border-border rounded-sm bg-surface p-8 hover:border-accent/40 transition-colors h-full">
          ...
          <Button
            onClick={onRequestAssessment}
            className="bg-highlight text-highlight-foreground hover:bg-highlight/90 rounded-sm px-6 text-sm tracking-wide"
          >
            Request Assessment
          </Button>
        </div>
...
const Closing = ({ onRequestAssessment }: { onRequestAssessment: () => void }) => (
  <section className="px-6 py-32 text-center max-w-3xl mx-auto relative">
    <FadeIn>
      {/* Ghosted typographic element */}
      <p className="absolute top-12 left-1/2 -translate-x-1/2 font-serif text-[8rem] sm:text-[12rem] text-border/30 pointer-events-none select-none leading-none">
        C.
      </p>
      <h2 className="font-serif text-3xl sm:text-4xl md:text-5xl font-normal italic leading-tight mb-12 relative z-10">
        What is not structured cannot endure.
      </h2>
      <Button
        onClick={onRequestAssessment}
        className="bg-highlight text-highlight-foreground hover:bg-highlight/90 rounded-sm px-8 py-3 text-sm tracking-wide relative z-10"
      >
        Request an Assessment
      </Button>
    </FadeIn>
